...
# ============================================================
SERIAL_BAUD      = 9600
# Phone IP-camera stream URL (Android "IP Webcam" app -> /video)
CAMERA_SOURCE    = "http://192.168.86.61:8080/video"
FRAME_W          = 640
FRAME_H          = 480
WEB_PORT         = 5000
...
class CameraThread(threading.Thread):
    ...
    def run(self):
        while state.running:
            log.info(f"Connecting to camera: {CAMERA_SOURCE}")
            ...
            if not cap.isOpened():
                state.cam_ok = False
                log.error("Camera unreachable, retrying in 2s...")
