...
+ Ollama vision narration/steering + Flask dashboard
Talks to Elegoo V4.0 firmware (F/B/L/R/S @ 115200).
"""
...
CAMERA_PATHS     = ["/video", "/videofeed", "/mjpegfeed", ""]
FRAME_W          = 640
FRAME_H          = 480
WEB_PORT         = 5000
...
OBSTACLE_THRESH_STOP = 0.45
OBSTACLE_THRESH_SLOW = 0.25
...
AI_SUGGESTION_TTL = 8.0
AI_TURN_BURST     = 0.40
AI_TURN_COOLDOWN  = 1.20
...
class ArduinoSerial:
    def __init__(self, port):
        ...
        try:
            ...
            self.send(f"SPEED:{INIT_SPEED}")
        except Exception as e:
            log.error(f"Serial failed: {e}")
        threading.Thread(target=self._reader, daemon=True).start()

    def send(self, cmd):
        code = CMD_MAP.get(cmd.strip().upper(), cmd.strip())
...
# ============================================================
# Camera Thread (wireless phone stream, auto-find path + reconnect)
# ============================================================
...
# ============================================================
# Ollama Thread (thinking brain - narration + steer suggestion)
# ============================================================
...
# ============================================================
# Navigation Thread - vision decisions
# ============================================================
class NavigationThread(threading.Thread):
    ...
    def run(self):
        log.info("Navigation: waiting for camera...")
        while state.running and not frame_is_fresh():
            time.sleep(0.1)
        time.sleep(2.0)
